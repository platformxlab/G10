...
__global__ void concat_backward(long n, long out_c, long h, long w, long num_input, long *c,
    ...
    for (long n = 0; n < out_c; n += parallel_size) {
            ...
            input[input_idx][idx - current_tail] = output[idx];
    }
}

Concat_Backward::Concat_Backward(cudnnHandle_t handle, vector<double> &args, bool is_UVM) : 
    ...
    if (!is_UVM) {
        ...
        GPUFillRand(output_data, (long) input_n * output_c * input_h * input_w * sizeof(float));
...
float Concat_Backward::Run() {
    if (is_UVM) {
        for (int i = 0; i < num_input; i++) {
            CUDA_CALL(cudaMallocManaged(&input_data[i], (long) input_n * input_cs[i] * input_h * input_w * sizeof(float)));
            GPUFillRand(input_data[i], (long) input_n * input_cs[i] * input_h * input_w * sizeof(float) * output_ratio);
        }
        CUDA_CALL(cudaMallocManaged(&output_data, (long) input_n * output_c * input_h * input_w * sizeof(float)));
        CPUFillRand(output_data, (long) input_n * output_c * input_h * input_w * sizeof(float));
        GPUFillRand(output_data, (long) input_n * output_c * input_h * input_w * sizeof(float) * input_ratio);
        CUDA_CALL(cudaMemcpy(device_input_data, input_data, (long) num_input * sizeof(float *), cudaMemcpyHostToDevice));
        cudaDeviceSynchronize();
    }
    ...
    concat_backward<<<input_n, ceil((double) output_c / input_n)>>>(input_n, output_c, input_h, input_w, num_input, device_input_cs, device_input_data, output_data);
