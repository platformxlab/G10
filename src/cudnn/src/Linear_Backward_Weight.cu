...
__global__ void backward_weight(int m, int n, int k, 
...
Linear_Backward_Weight::Linear_Backward_Weight(cudnnHandle_t handle, vector<double> &args, bool is_UVM) : 
    ...
    if (!is_UVM) {
        ...
        CUDA_CALL(cudaMalloc(&d_weight_data, (long) h_in * h_out * sizeof(float)));
...
Linear_Backward_Weight::~Linear_Backward_Weight() {
    if (!is_UVM) {
      CUDA_CALL(cudaFree(input_data));
      CUDA_CALL(cudaFree(d_weight_data));
      CUDA_CALL(cudaFree(d_output_data));
    }
}

float Linear_Backward_Weight::Run() {
    cublasHandle_t handle;
    cublasCreate(&handle);
    if (is_UVM) {
        CUDA_CALL(cudaMallocManaged(&input_data, (long) reshape * h_in * sizeof(float)));
        CUDA_CALL(cudaMallocManaged(&d_weight_data, (long) h_in * h_out * sizeof(float)));
        ...
        GPUFillRand(d_weight_data, (long) h_in * h_out * sizeof(float) * output_ratio);
        cudaDeviceSynchronize();
    }
    ...
    // backward_weight<<<GridSize, BlockSize>>>(h_in, h_out, reshape, input_data, d_output_data, d_weight_data);
    cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, h_in, h_out, reshape, &alpha, input_data, h_in, d_output_data, reshape, &beta, d_weight_data, h_in);
