...
Conv2d_Backward_Weight::Conv2d_Backward_Weight(cudnnHandle_t handle, vector<double> &args, bool is_UVM) : 
    ...
    CUDNN_CALL(cudnnCreateFilterDescriptor(&d_filter_descriptor));
    ...
    CUDNN_CALL(cudnnSetTensor4dDescriptor(
            ...
            output_n, output_c, output_h, output_w));
    // SetAlgorithm
    cudnnConvolutionBwdFilterAlgoPerf_t convolution_algo_perf;
    int algo_count;
    ...
    CUDNN_CALL(cudnnGetConvolutionBackwardFilterWorkspaceSize(
            handle,
            input_descriptor, d_output_descriptor, convolution_descriptor, d_filter_descriptor,
            algorithm,
            &workspace_size));
    // Alloc
    if (!is_UVM) {
        ...
        CUDA_CALL(cudaMalloc(&d_filter_data, (long) filter_n * filter_c * filter_h * filter_w * sizeof(float)));
        ...
        GPUFillRand(d_filter_data, (long) filter_n * filter_c * filter_h * filter_w * sizeof(float));
    }
    cudaDeviceSynchronize();
}

Conv2d_Backward_Weight::~Conv2d_Backward_Weight() {
    ...
    if (!is_UVM) {
        ...
        CUDA_CALL(cudaFree(d_filter_data));
        CUDA_CALL(cudaFree(workspace_data));
    }
}

float Conv2d_Backward_Weight::Run() {
    if (is_UVM) {
        ...
        CUDA_CALL(cudaMallocManaged(&d_filter_data, (long) filter_n * filter_c * filter_h * filter_w * sizeof(float)));
        ...
        CPUFillRand(d_filter_data, (long) filter_n * filter_c * filter_h * filter_w * sizeof(float));
        ...
        GPUFillRand(d_filter_data, (long) filter_n * filter_c * filter_h * filter_w * sizeof(float) * output_ratio);
        cudaDeviceSynchronize();
    }
    ...
    CUDNN_CALL(cudnnConvolutionBackwardFilter(
            ...
            &beta,
            d_filter_descriptor, d_filter_data));
...
size_t Conv2d_Backward_Weight::getWorkspaceSize() {
