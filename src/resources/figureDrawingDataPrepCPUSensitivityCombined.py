...
if __name__ == "__main__":
  ...
  for setting in ["deepUM", "FlashNeuron", "prefetch_lru"]:
    try:
      os.mkdir(f"figure_drawing/sensitivity_cpumem_combined/{setting}")
    ...
    for model in [INCEPTION, RESNET, SENET, VIT, BERT]:
      ...
      if not plot_format:
        x_ticks, y_ticks = [
          ...
          [list(cpu_mems.keys())[i] for i in y_axis_idxs]
        ]
      else:
        x_ticks, y_ticks = [
          [list(batch_sizes.keys())[i] for i in x_axis_idxs],
      ...
      if setting == "FlashNeuron":
        for x_idx in range(len(x_axis_idxs)):
          for y_idx in range(len(y_axis_idxs)):
            data_slice[x_idx, y_idx] = data_slice[x_idx, y_axis_idxs.index(cpu_mems["128"])]
      ...
      if latex_format:
        ...
        with open(f"figure_drawing/sensitivity_cpumem_combined/{setting}/{out_filename}.txt", "w") as f:
          print(f.name)
