...
if __name__ == "__main__":
  ...
  with open("figure_drawing/overall_traffic/traffic.txt", "w") as f:
    f.write("lru | FlashNeuron | deepUM | prefetch_lru\n\n")
    for model in [BERT, VIT, INCEPTION, RESNET, SENET]:
      ...
      f.write(model_desc + "\n")
      for stat_candidate in ["ssd2gpu_traffic", "gpu2ssd_traffic", "cpu2gpu_traffic", "gpu2cpu_traffic"]:
        ...
        if stat_candidate == "stall_percentage" or \
          stat_candidate == "overlap_percentage" or \
          stat_candidate == "compute_percentage":
          include_ideal = False
        if not plot_format:
          x_ticks, y_ticks = [
        ...
        for idx, d in np.ndenumerate(data_slice):
          if np.isclose(d, -1):
            data_slice[idx] = np.inf
        # data_slice *= 4096 / 1024 ** 3
        ...
        if latex_format:
          ...
          for x_idx, x_tick in enumerate(x_ticks):
            if not header_printed:
              slice_content += x_tick + "\n"
            slice_content += " ".join(str(d) if not np.isclose(float(d), -1) else "inf" for d in data_slice[x_idx, :])
            f.write(" ".join(str(d) if not np.isclose(float(d), -1) else "inf" for d in data_slice[x_idx, :]) + "\n")
        else:
          if normal_have_axis:
            slice_content += " " * 15
          ...
          for i in range(data_slice.shape[0]):
            ...
            for j in range(data_slice.shape[1]):
              slice_content += f"{data_slice[i, j]:15.5f}"
            slice_content += "\n"
        print(slice_content)
        header_printed = True
      f.write("\n")
