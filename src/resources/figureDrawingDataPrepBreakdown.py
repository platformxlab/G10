...
if __name__ == "__main__":
  ...
  for file in ["compute", "overlap", "stall"]:
    with open(f"figure_drawing/overall_breakdown/{file}.txt", "w") as f:
  ...
  for model in [BERT, VIT, INCEPTION, RESNET, SENET]:
    ...
    for stat_candidate in ["stall_percentage", "overlap_percentage", "compute_percentage"]:
      ...
      if latex_format:
        ...
        if stat_candidate == "stall_percentage":
          target = "stall"
        elif stat_candidate == "overlap_percentage":
          target = "overlap"
        elif stat_candidate == "compute_percentage":
          target = "compute"
        with open(f"figure_drawing/overall_breakdown/{target}.txt", "a") as f:
          f.write(model_desc + "\n")
        ...
        for i in range(data_slice.shape[0]):
          ...
          for j in range(data_slice.shape[1]):
            slice_content += f"{data_slice[i, j]:15.0f}"
          slice_content += "\n"
      print(slice_content)
      header_printed = True
