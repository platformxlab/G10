...
data_file = f"sensitivity_variation/all.txt"
...
ax0.set_xticklabels(["\n".join(workload.split("|")[0:2]) for workload in workloads])
...
ax0.set_yticks(np.arange(0.98, 1.006, 0.005))
...
labels = [f"$\pm${label}" if label != "0%" else label for label in labels]
legend = ax0.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.5, 1.05), ncol=5)
...
fig.savefig(f"output/SensitivityVariation.png", bbox_inches=extent)
fig.savefig(f"output/SensitivityVariation.pdf", bbox_inches=extent)
