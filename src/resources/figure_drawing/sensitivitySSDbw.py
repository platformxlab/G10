...
for model_idx in range(len(models)):
  ...
  for section_idx, section in enumerate(sections[1:]):
    ...
    data_array[section_idx, :] = np.array([second_dim_arr[-1] / float(data) for data in lines[1].split()])
  for j, setting in enumerate(first_dim_arr[:-1]):
    plot_idxs = data_array[:, j] != 0
    x_arr = [second_dim_arr[i] for i in range(len(plot_idxs)) if plot_idxs[i]]
    y_arr = [data_array[i, j] / data_array[i, -1] * 100 for i in range(len(plot_idxs)) if plot_idxs[i]]
    ax.plot(x_arr, y_arr, linestyle="--", color=color_arr[j], marker=marker_arr[j], markerfacecolor="none", label=first_dim_arr[j], linewidth=2, markersize=8)
  
  xlim = ax.get_xlim()
  ax.hlines(y=100, xmin=ax0.get_xlim()[0], xmax=ax0.get_xlim()[1], colors="grey", linestyles="--")
  ax.set_xticks(second_dim_arr)
  ax.set_xticklabels([str(d) for d in second_dim_arr])
  ax.set_xlabel(f"SSD Bandwidth (GB/s)", fontsize=15)
  plt.text(0.5, -0.27, f"({chr(ord('a') + model_idx)}) {list(net_name_translation.values())[model]}", ha="center", transform=ax.transAxes)
  ax.set_xlim(xlim)
...
ax0.set_ylabel("Normalized Performance")
# ax0.yaxis.set_label_coords(-0.22, 0.43)

handles, labels = ax.get_legend_handles_labels()
legend = fig.legend(handles, labels, frameon=False, loc="upper center", bbox_to_anchor=(0.5, 0.355), ncol=5)
...
fig.savefig(f"output/OverallPerfSSDBW.png", bbox_inches=extent)
fig.savefig(f"output/OverallPerfSSDBW.pdf", bbox_inches=extent)
